...
import org.imsouhay.Grove.ui.virtual.FakeScreenHandler;
import org.imsouhay.Grove.ui.virtual.sign.VirtualSignBlockEntity;
...
public class SignGui implements GuiInterface {
     ...
     *
     * @param player the player to serve this ui to
     */
    public SignGui(ServerPlayer player)  {
        this.player = player;
        int y= (int) player.getY();
        if(y>player.level().getMaxBuildHeight()) y=player.level().getMaxBuildHeight();
        if(y<player.level().getMinBuildHeight()) y=player.level().getMinBuildHeight();

        this.signEntity = new VirtualSignBlockEntity(new BlockPos(player.getBlockX(), y, player.getBlockZ()), Blocks.OAK_SIGN.defaultBlockState());
    }
     ...
     * This requires closing and reopening the ui, causing a flicker.
     */
    ...
    public void close(boolean alreadyClosed) {
        if (this.open && !this.reOpen) {
            ...
            this.player.connection.send(new ClientboundBlockUpdatePacket(signEntity.getBlockPos(), this.getPlayer().level().getBlockState(signEntity.getBlockPos())));
            this.onClose();
        } else {
            this.reOpen = false;
            this.open();
