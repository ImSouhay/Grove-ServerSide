...
public class IvEditButton {
    ...
    private static Button buildButton(Stat stat, ItemStack displayItem, PokeBuilder builder) {
        ...
        return GooeyButton.builder()
                .title("§a"+format(stat.getIdentifier().getPath())+" IV")
                .lore(buildLore(stat.getIdentifier().getPath()))
                .onClick(e -> {
                    if(e.getClickType()== ButtonClick.LEFT_CLICK) {handleLeftClick(e, stat, builder);}
                    else if(e.getClickType()== ButtonClick.RIGHT_CLICK){handleRightClick(e, stat, builder);}
                })
                .display(displayItem)
                .build();
    }
    ...
    private static ArrayList<String> buildLore(String statName) {
        String defaultLore="Click to open the @incORdec @statName IVs menu!".replace("@statName", format(statName));
    ...
    private static void handleLeftClick(ButtonAction e, Stat stat, PokeBuilder builder) {
        RoutingUtils.toStatEditor(e, builder,Destination.MAINANDIV, vType.IV, stat, Operation.PLUS, CobblemonItems.TIMID_MINT);
    }

    private static void handleRightClick(ButtonAction e, Stat stat, PokeBuilder builder) {
        RoutingUtils.toStatEditor(e, builder,Destination.MAINANDIV, vType.IV, stat, Operation.MINUS, CobblemonItems.BRAVE_MINT);
    }
}
